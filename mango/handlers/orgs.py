...
from mango.messages import orgs as models
from tornado import httpclient
from mango.system import orgs
...
# on org and just on org because of the natural stuff in the resource
# we're disable this @content_type_validation decorator

#@content_type_validation

class Handler(orgs.Org, BaseHandler):
    '''
    ...
    def put(self, org_uuid):
        ...
        if not format_pass:
            ...
            return
        account = self.request.arguments.get('account', [None])[0]
        result = yield self.replace_org(account, org_uuid, struct)
        if not result:
            self.set_status(400)
            system_error = errors.Error('missing')
            error = system_error.missing('org', org_uuid)
